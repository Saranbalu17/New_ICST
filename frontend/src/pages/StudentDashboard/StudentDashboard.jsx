...
import { BookOpen, Clock, CheckCircle, Search, Filter, ChevronRight, CreditCard, Trash2 } from 'lucide-react';
...
const CourseCard = ({ course, handleAddCard }) => {
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
      try {
        ...
        setCourses(Array.isArray(data?.listofStudents) ? data.listofStudents : []);
      } catch (err) {
  ...
  const fetchPopupdata = async () => {
    ...
    try {
      ...
      if (data.cardDetailsList?.length > 0) {
        setAddedCourses(Array.isArray(data?.cardDetailsList) ? data.cardDetailsList : []);
      } else {
        setAddedCourses([]);
      }
    } catch (error) {
  ...
  const handleDeleteCard = async (course) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        fetchPopupdata(); // Refresh cart data with VIEW flag
      } else {
  ...
  const handleConfirmPurchase = async (cartId, grandTotal) => {
    ...
    try {
      ...
      navigate('/payment-gateway', { state: { paymentData: data?.paymentGatewayResponse || {} } });
    } catch (error) {
      console.error('API Error (Confirm Purchase):', error);
      navigate('/payment-gateway', { state: { errorMessage: 'Failed to fetch payment details. Please try again.' } });
    }
  };
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {filteredCourses.map((course, index) => (
                <CourseCard key={index} course={course} handleAddCard={handleAddCard} />
              ))}
            </div>
          )}
        </div>
      ...
      {isCartOpen && (
        <div className="fixed inset-0 bg-black/40 z-50 flex justify-center items-center backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl p-6 animate-fadeIn relative">
            ...
            <h2 className="text-xl font-bold mb-4 text-gray-800">Your Selected Courses</h2>

            {Array.isArray(addedCourses) && addedCourses.length === 0 ? (
              <p className="text-gray-500">No courses added yet.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[450px] overflow-y-auto">
                {Array.isArray(addedCourses) && addedCourses.map((course, index) => (
                  <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg transform hover:-translate-y-1">
                    <div className="relative h-40 bg-gradient-to-r from-blue-500 to-purple-600">
                      <div className="absolute inset-0 flex items-center justify-center">
                        <img src="/api/placeholder/400/320" alt={course?.courseName || 'Course'} className="w-full h-full object-cover opacity-20" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between">
                        <h1 className="text-gray-600 font-bold">Amount: ₹{course?.amount || "0.00"}</h1>
                        <button
            ...
            <div className="mt-6 flex justify-between items-center">
              <button
                onClick={() => handleConfirmPurchase(addedCourses[0]?.cartId, addedCourses[0]?.grandTotal)}
                className="bg-green-600 hover:bg-green-700 font-bold text-white px-5 py-2 rounded-md transition"
                disabled={!Array.isArray(addedCourses) || addedCourses.length === 0}
              >
