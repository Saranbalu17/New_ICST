...
const UploadVideo = () => {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const res = await axios.post('https://beesprod.beessoftware.cloud/CloudilyaAPILMS/LMS/LMS_UploadedCourseLevelVideosForEmployee', formData, {
  ...
  useEffect(() => {
    const fetchAllPrograms = async () => {
      try {
        const response = await axios.post('https://beesprod.beessoftware.cloud/CloudilyaAPILMS/LMS/LMS_DisplayForEmployeeLevelPrograms', { grpCode, colCode, collegeId, EmployeeId: 4, ProgramId: 0, RegulationId: 0, BranchId: "", SemId: 0, SectionId: 0, CourseId: 0, TopicId: 0, UnitId: 0, Flag: 'PROGRAMS' })
        const { facultyDropdownList } = response.data
        setPrograms(facultyDropdownList)
      } catch (error) {
        console.log(error);
      }
    }
  ...
  useEffect(() => {
    if (!course) {
      ...
      const fetchAllBranches = async () => {
        try {
          setBranch('')
          const response = await axios.post('https://beesprod.beessoftware.cloud/CloudilyaAPILMS/LMS/LMS_DisplayForEmployeeLevelPrograms', { grpCode, colCode, collegeId, EmployeeId: 4, programId: course, RegulationId: 0, BranchId: "", SemId: 0, SectionId: 0, CourseId: 0, TopicId: 0, UnitId: 0, Flag: "BRANCHES" })
          const { facultyDropdownList } = response.data
          setProgrambasedBranch(facultyDropdownList)
        } catch (error) {
          console.log(error);
        }
      }
  ...
  useEffect(() => {
    if (!course) {
      ...
      const fetchAllRegualation = async () => {
        try {
          const response = await axios.post('https://beesprod.beessoftware.cloud/CloudilyaAPILMS/LMS/LMS_DisplayForEmployeeLevelPrograms', { grpCode, colCode, collegeId, EmployeeId: 4, programId: course, RegulationId: 0, BranchId: branch, SemId: 0, SectionId: 0, CourseId: 0, TopicId: 0, UnitId: 0, Flag: "REGULATION" })
          const { facultyDropdownList } = response.data
          setProgrambasedRegulation(facultyDropdownList)
        } catch (error) {
          console.log(error);
        }
      }
  ...
  useEffect(() => {
    if (!course || !branch || !regulation) {
      return;
    }
    else {
      const fetchAllSemester = async () => {
        try {
          const response = await axios.post('https://beesprod.beessoftware.cloud/CloudilyaAPILMS/LMS/LMS_DisplayForEmployeeLevelPrograms', { grpCode, colCode, collegeId, EmployeeId: 4, programId: course, RegulationId: Number(regulation), BranchId: branch, SemId: 0, SectionId: 0, CourseId: 0, TopicId: 0, UnitId: 0, Flag: "SEM" })
          const { facultyDropdownList } = response.data
          setProgrambasedSem(facultyDropdownList)
        } catch (error) {
          console.log(error);
        }
      }
  ...
  useEffect(() => {
    if (!course || !branch || !regulation || !semester) {
      ...
      const fetchAllSections = async () => {
        try {
          const response = await axios.post('https://beesprod.beessoftware.cloud/CloudilyaAPILMS/LMS/LMS_DisplayForEmployeeLevelPrograms', { grpCode, colCode, collegeId, EmployeeId: 4, programId: course, RegulationId: Number(regulation), BranchId: branch, SemId: Number(semester), SectionId: 0, CourseId: 0, TopicId: 0, UnitId: 0, Flag: "SECTION" })
          const { facultyDropdownList } = response.data
          setSections(facultyDropdownList)
        } catch (error) {
          console.log(error);
        }
      }
  ...
  }, [course, regulation, branch, semester])

  //coursebasedonprogramcourse


  useEffect(() => {
    if (!course || !branch || !regulation || !semester || !section) {
      return;
    }
    else {
      const fetchAllCourses = async () => {
        try {
          const response = await axios.post('https://beesprod.beessoftware.cloud/CloudilyaAPILMS/LMS/LMS_DisplayForEmployeeLevelPrograms', { grpCode, colCode, collegeId, EmployeeId: 4, programId: course, RegulationId: Number(regulation), BranchId: branch, SemId: Number(semester), SectionId: 0, CourseId: 0, TopicId: 0, UnitId: 0, Flag: "COURSES" })
          const { facultyDropdownList } = response.data
          setProgrambasedSubjects(facultyDropdownList)
        } catch (error) {
          console.log(error);
        }
      }
  ...
  }, [course, branch, regulation, semester, section])

  //unitsfetching  

  useEffect(() => {
    if (!course || !branch || !regulation || !semester || !section || !courseId) {
      return;
    }
    else {
      const fetchAllUnits = async () => {
        try {
          const response = await axios.post('https://beesprod.beessoftware.cloud/CloudilyaAPILMS/LMS/LMS_DisplayForEmployeeLevelPrograms', { grpCode, colCode, collegeId, EmployeeId: 4, programId: course, RegulationId: Number(regulation), BranchId: branch, SemId: Number(semester), SectionId: 0, CourseId: courseId, TopicId: 0, UnitId: 0, Flag: "UNITS" })
          const { facultyDropdownList } = response.data

          setUnitWise(facultyDropdownList)
        } catch (error) {
          console.log(error);
        }
      }
  ...
  }, [course, branch, regulation, semester, courseId, section])

  useEffect(() => {
    if (!course || !branch || !regulation || !semester || !courseId || !section || !unit) {
      return;
    }
    else {
      const fetchAllUnitsTopics = async () => {
        try {
          const response = await axios.post('https://beesprod.beessoftware.cloud/CloudilyaAPILMS/LMS/LMS_DisplayForEmployeeLevelPrograms', { grpCode, colCode, collegeId, EmployeeId: 4, programId: course, RegulationId: Number(regulation), BranchId: branch, SemId: Number(semester), SectionId: 0, CourseId: courseId, TopicId: 0, UnitId: Number(unit), Flag: "TOPICS" })
          const { facultyDropdownList } = response.data
          setTopicWise(facultyDropdownList)
        } catch (error) {
          console.log(error);
        }
      }
  ...
  }, [course, branch, regulation, semester, courseId, unit])
  ...
  const handelCourseChange = (e) => {
    ...
    setCourseId(Number(x.id))
    setCourseCode(x.code)
  }
                  ...
                  <div>
                    ...
                    >
                      ...
                      {programbasedBranch && programbasedBranch.map((programBranch) => (
                        <option value={programBranch.branchId} key={programBranch.branchId}>
                          {programBranch.branchName}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {programbasedSubjects && programbasedSubjects.map((programSubject) => (
                        <option value={JSON.stringify({ id: programSubject.courseId, code: programSubject.courseCode })} key={programSubject.courseCode}>
